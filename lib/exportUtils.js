...
const safeFetch = async (url, retries = 2) => {
    ...
    // Handle data URLs immediately
    if (url.startsWith('data:')) {
        try {
            const res = await fetch(url);
            return await res.arrayBuffer();
        } catch (err) {
            console.error(`[safeFetch] Data URL fetch failed:`, err);
            return null;
        }
    }
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            console.warn(`[safeFetch] Fetch failed for ${targetUrl} with status: ${res.status}`);
        } catch (err) {
            console.error(`[safeFetch] Attempt ${attempt} failed for ${targetUrl}:`, err);
            if (attempt < retries) {
...
/**
 * Detects the image extension from a URL.
 * Falls back to 'jpeg' for unknown / query-only extensions.
 */
const getExtFromUrl = (url, defaultExt = 'jpg') => {
    if (!url) return defaultExt;
    const raw = url.split('?')[0].split('.').pop().toLowerCase();
    return ['jpg', 'jpeg', 'png', 'webp', 'gif', 'svg'].includes(raw) ? raw : defaultExt;
};
...
const buildSpaceReadme = (moodboard, project, stats) => {
    ...
    return [
        ...
        '  CONTENTS',
        '-'.repeat(60),
        `  • ${sanitize(moodboard?.moodboard_name, 'space')}-report.xlsx  — Full material spec sheet with images`,
        `  • ${sanitize(moodboard?.moodboard_name, 'space')}-materials.csv — Lightweight CSV for spreadsheet apps`,
        ...
        stats.hasCover     ? `  • cover.*     — Space cover image` : null,
        '',
        '-'.repeat(60),
        '  ESTIMATED COST',
        '-'.repeat(60),
...
const buildProjectReadme = (project, moodboards, grandTotal) => {
    ...
    return [
        ...
        '  CONTENTS',
        '-'.repeat(60),
        `  • <project-name>-summary.xlsx — Multi-sheet project report`,
...
const appendMoodboardData = async (worksheet, moodboard, project, products = [], startRow) => {
    ...
    if (renders.length > 0) {
        ...
        rHeader.getCell(1).value = `VISUAL RENDERS — ${(moodboard.moodboard_name || '').toUpperCase()}`;
        rHeader.getCell(1).font  = { bold: true, color: { argb: COLORS.PRIMARY } };
        rHeader.getCell(1).fill  = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF0F4FF' } };
        rHeader.getCell(1).alignment = { horizontal: 'center', vertical: 'middle' };
        currentRow++;

        for (let ri = 0; ri < renders.length; ri++) {
            const render = renders[ri];
            const row = worksheet.getRow(currentRow);
            row.height = 130;
            row.getCell(2).value = render.title || `Render ${ri + 1}`;
            Object.assign(row.getCell(2), STYLES.cellLeft);

            if (render.previewUrl) {
                const buf = await safeFetch(render.previewUrl);
                if (buf) {
                    try {
                        const ext = getExtFromUrl(render.previewUrl, 'jpeg');
                        const imageId = workbook.addImage({ buffer: buf, extension: ext === 'jpg' ? 'jpeg' : ext });
                        worksheet.addImage(imageId, {
                            tl: { col: 0, row: currentRow - 1 },
...
const buildProjectSummaryExcel = async (project, moodboards) => {
    try {
        ...
        // ── Overview sheet ────────────────────────────────────────────────
        const ov = workbook.addWorksheet('Project Overview');
 ...
 */
const addSpaceToZip = async (zipFolder, moodboard, project, toastId) => {
    ...
    if (allUniqueProducts.length > 0) {
        toast.loading(`🖼 Fetching ${allUniqueProducts.length} product image(s) for "${spaceName}"…`, { id: toastId });
    ...
    // ── 4. Renders folder ─────────────────────────────────────────────────
    if (renders.length > 0) {
        toast.loading(`🎨 Fetching ${renders.length} render(s) for "${spaceName}"…`, { id: toastId });
    ...
    if (photos.length > 0) {
        toast.loading(`📷 Fetching ${photos.length} photo(s) for "${spaceName}"…`, { id: toastId });
    ...
    // ── 6. Canvas / Cover image ───────────────────────────────────────────
    const coverUrl = typeof moodboard?.coverImage === 'string' ? moodboard.coverImage.trim() : null;
    if (coverUrl) {
        console.log(`[addSpaceToZip] Fetching cover image for "${spaceName}"...`);
        const buf = await safeFetch(coverUrl);
        if (buf) {
            console.log(`[addSpaceToZip] Cover image fetched successfully for "${spaceName}"`);
            const ext = getExtFromUrl(coverUrl, 'jpg');
            // Save as both 'cover' and 'design-canvas' to be clear
            zipFolder.file(`cover.${ext}`, buf);
            zipFolder.file(`design-canvas.${ext}`, buf);
        } else {
            console.warn(`[addSpaceToZip] Failed to fetch cover image for "${spaceName}"`);
        }
    } else {
        console.warn(`[addSpaceToZip] No cover image found for "${spaceName}"`);
    }
 ...
 */
export const exportMoodboardToZip = async (moodboard, project, products = []) => {
    ...
    try {
        ...
        await addSpaceToZip(spaceFolder, moodboard, project, toastId);
        ...
        console.error('[ArcMat] Moodboard ZIP export failed:', err);
        toast.error('Failed to export space — please try again', { id: toastId });
    }
};
    ...
    try {
        ...
        if (projectCoverUrl) {
            ...
            if (buf) {
                ...
                projectFolder.file(`cover.${ext}`, buf);
            }
        }
        ...
        if (summaryBuffer) {
            projectFolder.file(`${projectName}-summary.xlsx`, summaryBuffer);
        }
...
/** @deprecated Use exportMoodboardToZip instead. */
...
/** @deprecated Use exportProjectToZip instead. */
