...
export const getImageUrl = (image, folder = 'product') => {
    ...
    if (typeof image === 'string') {
        ...
        // If the path already contains the folder or 'uploads', adjust construction
        if (cleanPath.includes('uploads/') || cleanPath.includes(`${folder}/`)) {
            // Ensure we don't double up on the base path if it's already there
            ...
            // If it's just 'folder/filename.jpg', it might need 'uploads/' prefix
            return `${apiUrl}/public/uploads/${cleanPath}`;
        }
