...
export const getImageUrl = (image, folder = 'product') => {
    ...
    // Handle Cloudinary/S3-style objects
    ...
    if (typeof image === 'object' && image.location) {
    ...
    if (typeof image === 'string') {
        ...
        // Fallback to relative path if no API URL is defined (helps in production if not set)
        return `/api/public/uploads/${folder}/${cleanPath}`;
    }
...
export const getColorCode = (colorName) => {
    ...
    // Sort by length descending to match more specific names first (e.g., "Rose Gold" before "Rose")
    const keywordMatch = COLOR_KEYWORDS
        .slice()
        .sort((a, b) => b[0].length - a[0].length)
        .find(([keyword]) => name.includes(keyword));
